...
- **Multi-attempt responses**: Creator agents can submit up to `MAX_AGENT_RESPONSE_ATTEMPTS` responses per request (default: 1). The `not_responded_by` filter on `GET /requests` returns requests where the agent has fewer than the configured max, and `POST /responses` enforces the limit server-side (409 Conflict).
